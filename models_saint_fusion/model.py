...
class RowColTransformer(nn.Module):
    def __init__(self, num_tokens, dim, nfeats, depth, heads, dim_head, attn_dropout, ff_dropout,style='col'):
        ...
        self.style = style
        self.project_384_to_32 = nn.Sequential(
            nn.Linear(384, 32),
            nn.GELU(),
            nn.LayerNorm(32),
            nn.Dropout(0.1) # A small, healthy dropout just for the text vector
        )
    ...
    def forward(self, x, x_cont=None, mask = None):
        tab_emb = torch.cat([x, x_cont], dim=1)
        # Should return True
        torch.allclose(tab_emb[:, :39, :], x)
        torch.allclose(tab_emb[:, 39:, :], x_cont)
        text_emb_32 = self.project_384_to_32(text_emb)
